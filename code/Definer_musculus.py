...
def analyze(temp, OutputDir):
    ...
    file = open(OutputDir + '/result对比实验one-hot.txt', 'w')
    index = 0
...
def funciton(PositiveCSV, NegativeCSV, OutputDir, folds, valib_P=None, valib_N=None):
    ...
    for test_index in range(folds):
        ...
        model_check = ModelCheckpoint(filepath = OutputDir + "/model" + str(test_index+1) +".h5",
                                      monitor = 'val_binary_accuracy', save_best_only=True, save_weights_only=False
                                      )
        ...
        model = load_model('../Definer_model/musculus_result/model10.h5')
        # print(model.summary())
...
def out_(valib_P=None, valib_N=None):
    valib_P = "../test/valib_P.txt"
    valib_N = "../test/valib_N.txt"
    # print(valib_P, valib_N)
    NegativeCSV = r"..\data\musculus_negitive_472.txt"
    PositiveCSV = r"..\data\musculus_positive_472.txt"

    OutputDir = r"..\Definer_model\musculus_result"
    # print("调用了函数；")
    # funciton(PositiveCSV, NegativeCSV, OutputDir, 10, valib_P, valib_N);
