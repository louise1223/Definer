...
def calculateScore(X, y, model, folds):
    ...
    return {'sn' : sensitivity, 'sp' : specificity, 'acc' : accuracy, 'MCC' : MCC}
...
def funciton(PositiveCSV, NegativeCSV, OutputDir, folds, valib_P=None, valib_N=None):
    ...
    validation_result, trainning_result, testing_result = [], [], []
    # 导入数据处理
    ...
    for test_index in range(folds):
        ...
        model = load_model('../Definer_model/cerevisiae_result/model10.h5')
...
def out_(valib_P=None, valib_N=None):
    ...
    NegativeCSV = r"..\data\cerevisiae_negitive_314.txt"
    PositiveCSV = r"..\data\cerevisiae_positive_314.txt"

    OutputDir = r"..\Definer_model\cerevisiae_result"
    # print("调用了函数；")
